...
async function handleMessage(event) {
  ...
  if (/^[0-9]{6}$/.test(text) || /^[a-f0-9]{32}$/i.test(text)) {
    await handleLinkByOTP(lineUserId, text, event.replyToken)
    return
  }

  // คำสั่งอื่น
  if (text === 'งานวันนี้' || text === 'สรุปงาน') {
    await triggerDailySummaryForUser(lineUserId)
    return
  }
...
async function triggerDailySummaryForUser(lineUserId) {
  const result = await crmDB.query(`
    SELECT * FROM v_daily_summary_per_user WHERE line_user_id=$1
  `, [lineUserId])
  if (result.rows.length > 0) {
    await lineService.sendDailySummary(result.rows[0])
  }
}
